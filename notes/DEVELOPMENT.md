...
```shell
sed -i '' 's/"version": ".*"/"version": "0.22.0"/' ../package.json
grep '"version":' ../package.json
```
...
poetry run python -m scripts.update_changelog --repo-dir ~/projects/capital-copilot-fe 'v0.22.0' 'v0.21.1'
```
...
git tag -a v0.22.0 -m "In this update, we've introduced new features and improvements to the job service, podcast functionalities, and user interface." && \
