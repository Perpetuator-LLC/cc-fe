...
```shell
sed -i '' 's/"version": ".*"/"version": "0.13.2"/' ../package.json
grep '"version":' ../package.json
```
...
poetry run python -m scripts.update_changelog --repo-dir ~/projects/capital-copilot-fe-private 'v0.13.2' 'v0.13.1'
```
...
```shell
git add .. && \
git commit -m "chore: bump version to 0.13.2" && \
git push origin main && \
git tag -a v0.13.2 -m "In this patch update, we have fixed a loop in the article page and changed the default publish team to 'none' to make publishing more intentional." && \
