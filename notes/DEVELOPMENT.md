...
```shell
sed -i '' 's/"version": ".*"/"version": "0.20.0"/' ../package.json
grep '"version":' ../package.json
```
...
poetry run python -m scripts.update_changelog --repo-dir ~/projects/capital-copilot-fe 'v0.20.0' 'v0.19.0'
```
...
git push origin main && \
git tag -a v0.20.0 -m "This update primarily focuses on integrating Relay support, enhancing podcast and team functionalities, and refining transaction handling." && \
